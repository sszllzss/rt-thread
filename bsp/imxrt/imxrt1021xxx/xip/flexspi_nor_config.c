 ...
 * @Date: 2021-08-19 10:33:07
 * @LastEditors: SSZL
 * @LastEditTime: 2021-08-20 11:30:48
 */
/*
    ...
    .memConfig =
        {
            ...
            // .deviceModeArg = 0x000040,/*设置 QE=1（S6）*/
            ...
            .configCmdArgs[2] =  0x000040,/*设置 QE=1（S6）*/
            ...
            .lookupTable =
                {
                    ...
                    [4*CMD_LUT_SEQ_IDX_READ+1]     = FLEXSPI_LUT_SEQ(MODE8_SDR, FLEXSPI_4PAD, 0xA0, DUMMY_SDR, FLEXSPI_4PAD, 0x04),// MODE8_SDR里参数值填入0xA0
        ...
        //			[4*CMD_LUT_SEQ_IDX_READ+2]     = FLEXSPI_LUT_SEQ(READ_SDR, FLEXSPI_4PAD, 0x04, STOP, FLEXSPI_1PAD, 0x00),
                    ...
                    [4*CMD_LUT_SEQ_IDX_SECTORERASE+1]   = FLEXSPI_LUT_SEQ(STOP, FLEXSPI_1PAD, 0x00, STOP, FLEXSPI_1PAD, 0x00),
                    /* 启用复位 */
                    ...
                    /* 写状态寄存器RSR */
                    [4*CMD_LUT_SEQ_IDX_WRITESTATUS1_2+0]   = FLEXSPI_LUT_SEQ(CMD_SDR, FLEXSPI_1PAD, 0x01, WRITE_SDR, FLEXSPI_1PAD, 0x01),
                    /*擦除块命令*/
                    ...
                    [4*CMD_LUT_SEQ_IDX_EXITNOCOM+0]     = FLEXSPI_LUT_SEQ(RADDR_SDR, FLEXSPI_4PAD, 0x18, MODE8_SDR, FLEXSPI_4PAD, 0x00),// MODE8_SDR里参数值填入0x00
